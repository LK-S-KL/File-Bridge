...
0.6.9 的 **macOS** 包内置 ARM64 和 Intel 两套 FFmpeg / FFprobe，**Windows 暂不支持**。
...
项目数量旁的更多菜单提供“准备离线预览”：默认元数据与海报，可选雪碧图、540p/720p 播放代理，
支持范围、目录、预算、暂停/继续、取消和失败重试。默认低占用模式单重任务、单解码线程；
浏览优先显示本地预览，滚动时暂停新重任务，鼠标停留后才生成雪碧图。
打开播放预览会暂停批次，之后可手动继续；重启后任务停在暂停状态，不擅自继续转码。
周期性检查点避免每个素材都重写万项清单，崩溃后可能重新核对最近 100 项以内的产物。

缓存存放于默认本机目录或所选本地目录的专用子目录，图片区与代理区分开计费，保留离线项不自动淘汰。
原路径离线时只提供缓存预览，不把离线源文件导入 Adobe；插件播放代理不是 Pr 工程代理附件。
低占用播放优先使用缓存代理，已识别的高规格素材无代理时先生成 540p 代理，不自动批量复制所有源视频。
详见 [0.6.9 性能验收记录](docs/V0.6.9_MACOS_RELEASE.md)。
