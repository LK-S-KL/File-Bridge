...
const playerCssSource = fs.readFileSync(playerCssPath, "utf8");
...
assert.match(indexSource, /css\/player-4\.5\.css/, "The supplied player 4.5 visual layer must be loaded");
assert.match(indexSource, /id="speedButton"/, "The player must expose the supplied playback speed control");
assert.match(playerCssSource, /min-height:\s*132px/, "The player must preserve the 4.5 wide layout height");
assert.equal(fs.existsSync(playerCameraIconPath), true, "The supplied player camera icon must be packaged");
